...
public:
  ...
  virtual void save(const IDMap &m) const = 0;
  virtual void load()  = 0;
...
inline void
GPMatrix::weighted_update(double rho)
{
  if (rho == 1) {
    swap();
    return;
  }
  double **scurrd = _scurr.data();
  double **rcurrd = _rcurr.data();
  double **snextd = _snext.data();
  double **rnextd = _rnext.data();
  for (uint32_t i = 0; i < _n; ++i)
    for (uint32_t k = 0; k < _k; ++k) {
      scurrd[i][k] = (1 - rho) * scurrd[i][k] + rho * snextd[i][k];
      rcurrd[i][k] = (1 - rho) * rcurrd[i][k] + rho * rnextd[i][k];
    }
  set_to_prior();
}
...
inline void
GPMatrix::save(const IDMap &m) const
{
...
inline void
GPMatrixGR::weighted_update(double rho)
{
  if (rho == 1) {
    swap();
    return;
  ...
  for (uint32_t k = 0; k < _k; ++k)  
    rcurrd[k] = (1 - rho) * rcurrd[k] + rho * rnextd[k];
...
inline void
GPMatrixGR::save(const IDMap &m) const
{
  string expv_fname = string("/") + name() + ".tsv";
  string shape_fname = string("/") + name() + "_shape.tsv";
  string rate_fname = string("/") + name() + "_scale.tsv";
...
inline void
GPArray::weighted_update(double rho)
{
  if (rho == 1) {
    swap();
    return;
  ...
  for (uint32_t i = 0; i < _n; ++i) {
    scurrd[i] = (1 - rho) * scurrd[i] + rho * snextd[i];
    rcurrd[i] = (1 - rho) * rcurrd[i] + rho * rnextd[i];
  }
  set_to_prior();
}
...
inline void
GPArray::save(const IDMap &m) const
{
  string expv_fname = string("/") + name() + ".tsv";
  string shape_fname = string("/") + name() + "_shape.tsv";
  string rate_fname = string("/") + name() + "_scale.tsv";
