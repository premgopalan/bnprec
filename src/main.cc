...
main(int argc, char **argv)
{
  ...
  bool vb = true;
  bool natgrad_seq_updates = false;
  ...
  while (i <= argc - 1) {
    if (strcmp(argv[i], "-dir") == 0) {
      ...
      vb = false;
      natgrad_seq_updates = false;
    } else if (strcmp(argv[i], "-natgrad-seq") == 0) {
      natgrad_seq_updates = true;
      vb = false;
  ...
  Env env(n, m, k, fname, nmi, ground_truth_fname, rfreq, 
	  ...
	  batch, binary_data, bias, explore, alpha, 
	  vb, natgrad_seq_updates, kappa, scale);
  env_global = &env;
  if (p) {
    postprocess(env);
    exit(0);
