...
public:
  ...
  void lognormalize() { }
  void lognormalize(double logsum) { }
  void normalize();
...
template<> inline void
D1Array<double>::lognormalize(double logsum)
{
  for (uint32_t i = 0; i < _n; ++i)
    _data[i] = ::exp(_data[i] - logsum);
}
...
public:
  ...
  void load(string name) const { lerr("load not implemented"); }
  double abs_mean() const;
...
template<> inline void
D2Array<double>::load(string name) const
{
  FILE *f = fopen(name.c_str(), "r");
  if (!f)
    lerr("cannot open file %s\n", name.c_str());
