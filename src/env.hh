...
public:
  ...
  Env(uint32_t N, uint32_t M, uint32_t K, string fname, 
      ...
      bool bias, bool explore, double alpha,
      bool vb, bool natgrad_seq_updates, double kappa,
      double scale);
...
Env::Env(uint32_t N, uint32_t M, uint32_t K, string fname, 
	 ...
	 double alphav, 
	 bool vbv, bool natgrad_seq_updatesv, 
	 double kappav, double scalev)
  : dataset(datasetv),
    n(N),
    m(M),
    k(K),
    ...
    bias(biasv),
    vb(vbv),
    natgrad_seq_updates(natgrad_seq_updatesv),
    kappa(kappav),
    scale(scalev)
{
  ostringstream sa;
  ...
  plog("natgrad_seq_updates", natgrad_seq_updates);
  plog("vb", vb);
  plog("kappa", kappa);
  plog("tau0", tau0);
